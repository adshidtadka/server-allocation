...
int BGraph::hopcroftKarpAlgorithm() {
    ...
    for (int u = 0; u <= m; u++) pair_u[u] = NIL;
    for (int v = 0; v <= n; v++) pair_v[v] = NIL;
...
bool BGraph::bfs() {
    ...
    for (int u = 1; u <= m; u++) {
        ...
        // Else set distance as infinite so that this vertex is considered next
        // time for availibility
        else
            dist[u] = INF;
    }
    ...
    if (u != NIL) {
        ...
        for (it = adj[u].begin(); it != adj[u].end(); ++it) {
            ...
            if (dist[pair_v[v]] == dist[u] + 1) {
                ...
                if (dfs(pair_v[v]) ==
                    true) {  // new matching possible, store the matching
                    pair_v[v] = u;
                    pair_u[u] = v;
                    return true;
                }
